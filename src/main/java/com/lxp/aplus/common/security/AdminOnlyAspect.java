...
/**
 * @AdminOnly 어노테이션을 처리하는 AOP Aspect
 * <p>
 * 메서드 실행 전에 관리자 권한을 체크합니다.
 */
@Slf4j
@Aspect
@Component
...
public class AdminOnlyAspect {
    ...
    @Before("@annotation(com.lxp.aplus.common.security.AdminOnly)")
    public void checkAdminRole(JoinPoint joinPoint) {
        ...
        if (userId == null) {
            log.warn("@AdminOnly 메서드에 @Authenticated 파라미터가 없습니다: {}", joinPoint.getSignature());
        ...
        if (!hasAdminRole) {
            log.warn("관리자 권한이 없는 사용자가 관리자 전용 메서드에 접근 시도: userId={}, method={}",
                    userId, joinPoint.getSignature());
            throw new BusinessException(UserErrorCode.NOT_ADMIN);
        }
    }
